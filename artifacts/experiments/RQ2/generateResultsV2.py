...
REPORT_NAME = "artifacts/output/rq2_v2.csv"
TEX_REPORT_NAME_METHOD_COV = "artifacts/output/rq2_cov_v2.tex"
TEX_REPORT_NAME_TIME_TAKEN = "artifacts/output/rq2_time_v2.tex"

ITERATIONS = [10, 1000]
RAW_NAMES = ["Property", "10", "100", "1000"]
...
def generate_project_df(project_ds: dict[int, dict]) -> tuple[pd.DataFrame(), pd.DataFrame()]:
    property_dict = {}
    ...
    project_df_method_cov = pd.DataFrame()
    project_df_time_taken = pd.DataFrame()

    for prop, val in property_dict.items():
        print(val)
        mc_dict = {"Property": propertyShortNames[prop], 10: val[1][0],
                   100: val[0][0], 1000: val[2][0]}
        tt_dict = {"Property \emph{time(s)}": propertyShortNames[prop], 10: val[1][1],
                   100: val[0][1], 1000: val[2][1]}
        ...
        project_df_method_cov = pd.concat([project_df_method_cov, method_coverage_df])
        project_df_time_taken = pd.concat([project_df_time_taken, time_taken_df])
    return (project_df_method_cov, project_df_time_taken)
...
def main():
    ...
    with open(TEX_REPORT_NAME_METHOD_COV, 'w') as tf:
        df = pd.DataFrame()
        for project in PROJECTS:
            final_dataset[project][0]['_style'] = ''
            header = dict(zip(['Property', 10, 100, 1000], ['', '', '', '', '', '']))
            df = pd.concat([
                df,
                pd.DataFrame(header | {'_style': 'HEADER', 'Property': project}, index=[0]),
                final_dataset[project][0]
            ], ignore_index=True)
    ...
    with open(TEX_REPORT_NAME_TIME_TAKEN, 'w') as tf:
        df = pd.DataFrame()
        for project in PROJECTS:
            final_dataset[project][1]['_style'] = ''
            header = dict(zip(['Property \emph{time(s)}', 10, 100, 1000], ['', '', '', '', '', '']))
            df = pd.concat([
                df,
                pd.DataFrame(header | {'_style': 'HEADER', 'Property \emph{time(s)}': project}, index=[0]),
                final_dataset[project][1]
            ], ignore_index=True)
