...
for project in projects:
    ...
    data['linesTotal'] = data['linesCovered'] + data['linesMissed']

    # false-positives: tool identifies code as reachable,
    #   but cannot be reached by a property test
    data['FP'] = data[[ data['reachableJaCoCo'] & ~data['reachableProperty'] ]]['linesTotal']

    # false-negatives: code that is reachable from the property
    #   test but the tool does not identify it as such
    data['FN'] = data[[ ~data['reachableJaCoCo'] & data['reachableProperty'] ]]['linesTotal']

    # JaCoCo and our tool agree that is reachability
    data['TP'] = data[[ data['reachableJaCoCo'] & data['reachableProperty'] ]]['linesTotal']

    # JaCoCo and our tool agree that is NOT reachable
    data['TN'] = data[[ ~data['reachableJaCoCo'] & ~data['reachableProperty'] ]]['linesTotal']
    ...
    dfGrouped = data[[FIELD_PROPERTY, 'FP', 'FN', 'TP', 'TN']].groupby(by=FIELD_PROPERTY).sum().round(2)
